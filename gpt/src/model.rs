...
impl BigramLanguageModel {
    fn new(vocab_size: usize, vb: VarBuilder) -> Result<Self> {
        let token_embedding_table = embedding(vocab_size, N_EMBD, vb.pp("model.token_embd"))?;
        let position_embedding_table = embedding(BLOCK_SIZE, N_EMBD, vb.pp("model.pos_embd"))?;
        let sa_head = Head::new(N_EMBD, vb.pp("sa_head"))?;
