...
use candle_nn::{
    embedding, layer_norm, linear, linear_no_bias, loss::cross_entropy, ops::softmax_last_dim,
    Activation, Dropout, Embedding, LayerNorm, LayerNormConfig, Linear, Optimizer, VarBuilder,
    VarMap,
};
use rand::{distributions::Distribution, rngs::StdRng, Rng, SeedableRng};
use utils::masked_fill;

const BATCH_SIZE: usize = 32; //64;
const BLOCK_SIZE: usize = 8; //256;
const N_EMBD: usize = 32; //384;
const SEED: u64 = 1337;
const MAX_ITERS: usize = 5000;
const EVAL_INTERVAL: usize = 500;
const EVAL_ITERS: usize = 200;
const N_HEAD: usize = 4; //6;
const N_LAYER: usize = 4; //6;
const LEARNING_RATE: f64 = 1e-3; //3e-4;
